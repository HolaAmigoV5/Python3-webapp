...
async def select(sql,args,size=None):
	...
	global __pool
	async with __pool.get() as conn:  #使用with语句来自动帮我们调用close()方法
		async with conn.cursor(aiomysql.DictCursor) as cur: #调用协程获取cursor
			await cur.execute(sql.replace('?','%s'),args or ()) #将SQL语句的占位符?替换为MySql的占位符%s，并执行SQL
			if size:
